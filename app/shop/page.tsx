        ...
        <div className="flex flex-col lg:flex-row gap-10">
              ...
              <div className="flex items-start justify-between gap-4 border-b border-soft-border/70 pb-5">
                ...
                {(category || minPrice || maxPrice) && (
                  <button
                    ...
                    className="inline-flex items-center gap-1 text-[11px] uppercase tracking-[0.12em] text-soft-green transition-colors hover:text-gold"
                  >
                    <RotateCcw size={12} />
                    Clear
                  </button>
                  ...
                  <li>
                    <button
                      ...
                      className={cn(
                        ...
                        !category ? 'bg-forest text-cream' : 'text-soft-green hover:bg-muted-cream/70 hover:text-forest'
                      )}
                    >
                      <span>All categories</span>
                      {!category && <Check size={15} />}
                    </button>
                  </li>
                  {categories.map((c) => {
                    ...
                    return (
                      <li key={c._id}>
                        <button
                          ...
                          className={cn(
                            'flex w-full items-center justify-between gap-3 px-3 py-2.5 text-left text-sm transition-colors',
                            selected ? 'bg-forest text-cream' : 'text-soft-green hover:bg-muted-cream/70 hover:text-forest'
                          )}
                        >
                ...
                <div className="grid grid-cols-2 gap-2">
                  <label className="relative">
                    ...
                    <input type="number" placeholder="Min" defaultValue={minPrice}
                      className="input-luxury w-full py-2.5 pl-7 text-sm"
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-8">
              <button onClick={() => setFiltersOpen(!filtersOpen)} className="lg:hidden text-sm text-forest">
            ...
            {loading ? (
              <div className="grid grid-cols-2 md:grid-cols-3 gap-6 md:gap-8">
                {[...Array(6)].map((_, i) => (
                  <div key={i}><Skeleton className="aspect-[4/5]" /><Skeleton className="h-4 w-2/3 mt-4" /></div>
                ))}
              </div>
            ) : products.length === 0 ? (
                  ...
                  <div className="flex justify-center gap-2 mt-12">
                    {[...Array(pagination.pages)].map((_, i) => (
                      <button key={i} onClick={() => updateParams({ page: String(i + 1) })}
                        className={cn('w-10 h-10 text-sm transition-colors',
                          page === i + 1 ? 'bg-forest text-cream' : 'border border-soft-border text-forest hover:border-gold')}>
                        {i + 1}
                      </button>
                    ))}
                  </div>
