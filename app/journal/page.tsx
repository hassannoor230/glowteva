...
export default function JournalPage() {
  ...
  useEffect(() => {
    api.get<{ data: { journals: JournalArticle[] } }>('/journal')
      ...
      <div className="container-luxury">
        <div className="text-center mb-16">
          ...
          <p className="body-elegant mt-4 max-w-lg mx-auto">
            Botanical skincare, rituals, ingredients, and the quieter side of beauty.
          </p>
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[...Array(6)].map((_, i) => (
              <div key={i}><Skeleton className="aspect-[16/10]" /><Skeleton className="h-6 w-3/4 mt-4" /></div>
            ))}
          </div>
        ) : (
              ...
              <Link key={a._id} href={`/journal/${a.slug}`} className={`group ${i === 0 ? 'md:col-span-2 md:row-span-1' : ''}`}>
                <div className={`relative overflow-hidden mb-5 bg-muted-cream ${i === 0 ? 'aspect-[21/9]' : 'aspect-[16/10]'}`}>
                  <Image src={a.coverImage} alt={a.title} fill
                    className="object-cover transition-transform duration-700 group-hover:scale-105" sizes={i === 0 ? '66vw' : '33vw'} />
                </div>
                <p className="text-[11px] tracking-[0.15em] uppercase text-soft-green mb-2">
                  {a.category} · {a.readingTime} min read
                </p>
                <h2 className={`font-serif text-forest group-hover:text-gold transition-colors ${i === 0 ? 'text-2xl md:text-3xl' : 'text-xl'}`}>
                  {a.title}
                </h2>
