      ...
      <div className="container-luxury">
              ...
              <div className="flex gap-3">
                {images.map((img, i) => (
                  <button key={i} onClick={() => setActiveImage(i)}
                    className={`relative w-20 h-24 overflow-hidden border-2 transition-colors ${i === activeImage ? 'border-gold' : 'border-transparent'}`}>
          ...
          <div>
            ...
            <div className="flex items-center gap-4 mb-8 mt-8">
              <div className="flex items-center border border-soft-border">
                <button onClick={() => setQty(Math.max(1, qty - 1))} className="p-3 text-soft-green hover:text-forest" aria-label="Decrease">
                  <Minus size={16} />
                </button>
                <span className="px-4 text-forest">{qty}</span>
                <button onClick={() => setQty(Math.min(displayStock, qty + 1))} className="p-3 text-soft-green hover:text-forest" aria-label="Increase">
                  <Plus size={16} />
                </button>
              </div>
              <button
                onClick={() => {
                  ...
                  addItem(product, qty, selectedVariant ?? undefined, selectedOptions);
                }}
              ...
              <button onClick={toggleWishlist} className="p-3.5 border border-soft-border text-forest hover:text-gold transition-colors" aria-label="Wishlist">
              ...
              <div className="mb-8">
                ...
                <ul className="space-y-1">
                  {product.benefits.map((b) => (
                    <li key={b} className="text-sm text-soft-green flex items-center gap-2">
                      <span className="w-1 h-1 bg-gold rounded-full" />{b}
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-10 border-t border-soft-border pt-10">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {product.ingredients?.length > 0 && (
                  <p className="text-[10px] tracking-wider uppercase text-soft-green">{product.ingredients.length} botanical sources</p>
                )}
              </div>
            </div>
            {product.ingredients?.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {product.ingredients.map((ing) => (
                  <span key={ing} className="px-3 py-1.5 bg-muted-cream text-sm text-forest rounded-full border border-soft-border/50 hover:border-gold/50 transition-colors">{ing}</span>
                ))}
              </div>
            ) : (
          ...
          <div className="card-luxury p-6 space-y-4 relative overflow-hidden">
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-forest/10 flex items-center justify-center">
                <BookOpen className="text-forest" size={18} />
              </div>
              <div>
            ...
            {product.howToUse ? (
              <p className="text-soft-green text-sm leading-relaxed pl-2">{product.howToUse}</p>
            ) : (
              <p className="text-soft-green text-sm pl-2">No usage instructions available</p>
            )}
          </div>
          ...
          <div className="card-luxury p-6 space-y-4 relative overflow-hidden">
              ...
              <div>
                ...
                {product.rating > 0 ? (
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-xs text-soft-green">{product.rating.toFixed(1)} ({product.reviewCount})</span>
                  </div>
                ) : (
                  <p className="text-xs text-soft-green">No ratings yet</p>
                )}
              </div>
            </div>
            {reviews.length === 0 ? (
              <p className="text-soft-green text-sm">No reviews yet. Be the first to share your experience.</p>
            ) : (
                ...
                {reviews.map((r) => (
                  <div key={r._id} className="border-b border-soft-border pb-3 last:border-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {r.verifiedPurchase && (
                        <span className="text-[10px] tracking-wider uppercase text-gold bg-gold/10 px-1.5 py-0.5 rounded">Verified</span>
                      )}
                    </div>
            ...
            {reviews.length > 0 && (
              <button className="text-xs text-gold hover:text-forest transition-colors font-medium tracking-wide">Read all reviews →</button>
            )}
          </div>
        </div>
