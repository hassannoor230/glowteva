      ...
      <div className="container-luxury max-w-4xl">
        ...
        <div className="flex gap-6 border-b border-soft-border mb-10 overflow-x-auto">
          {tabs.map((t) => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`pb-3 text-sm tracking-wide whitespace-nowrap border-b-2 transition-colors ${
                tab === t.id ? 'border-gold text-forest' : 'border-transparent text-soft-green hover:text-forest'
              }`}>
              {t.label}
            </button>
          ))}
          ...
          <div className="space-y-8">
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-6">
              <div className="card-luxury p-6 text-center">
                <p className="text-3xl font-serif text-forest">{orders.length}</p>
                <p className="text-sm text-soft-green mt-1">Orders</p>
              </div>
              <div className="card-luxury p-6 text-center">
                <p className="text-3xl font-serif text-forest">{user.wishlist?.length || 0}</p>
                <p className="text-sm text-soft-green mt-1">Wishlist</p>
              </div>
              <div className="card-luxury p-6 text-center">
                <p className="text-3xl font-serif text-forest">{user.addresses?.length || 0}</p>
                <p className="text-sm text-soft-green mt-1">Addresses</p>
              </div>
              <div className="card-luxury p-6 text-center">
                <p className="text-3xl font-serif text-forest">{unreadNotifications}</p>
                <p className="text-sm text-soft-green mt-1">Notifications</p>
              </div>
            </div>
            {orders.length > 0 && (
              <div>
                ...
                {orders.slice(0, 3).map((o) => (
                  <div key={o._id} className="flex justify-between items-center py-4 border-b border-soft-border">
                    <div>
                      ...
                      <p className="text-xs text-soft-green">{formatDate(o.createdAt)} · {o.orderStatus}</p>
                    </div>
          ...
          <div>
            {loading ? <p className="text-soft-green">Loading...</p> : orders.length === 0 ? (
              <div className="text-center py-16">
                <p className="font-serif text-xl text-forest mb-2">No orders yet</p>
                  ...
                  <div key={o._id} className="card-luxury p-6">
                    <div className="flex flex-wrap justify-between gap-4 mb-4">
                      ...
                      <div className="text-right">
                        <p className="font-medium text-forest">{formatPrice(o.total)}</p>
                        <p className="text-xs text-soft-green">{o.orderStatus} · {o.paymentStatus}</p>
                      </div>
                    ...
                    <div className="space-y-2">
                      {o.items.map((item, i) => (
                        <div key={i} className="flex justify-between text-sm">
                          <span className="text-soft-green">{item.name} × {item.quantity}</span>
        ...
        {tab === 'profile' && (
          <div className="card-luxury p-8 max-w-md">
            ...
            <div className="space-y-4">
              <div>
                <p className="text-xs text-soft-green tracking-wide uppercase">Name</p>
                <p className="text-forest">{user.name}</p>
              </div>
              <div>
                <p className="text-xs text-soft-green tracking-wide uppercase">Email</p>
                <p className="text-forest">{user.email}</p>
              </div>
              <div>
                <p className="text-xs text-soft-green tracking-wide uppercase">Member since</p>
                <p className="text-forest">{user.createdAt ? formatDate(user.createdAt) : '—'}</p>
              </div>
            </div>
          </div>
        )}
